...
class Dataset:
    def __init__(self, batch_size, folder='data', include_hair=True):
        ...
        train_files, validation_files, test_files = self.train_valid_test_split(
            os.listdir(os.path.join(folder, 'inputs2')))
    ...
    def file_paths_to_images(self, folder, files_list, verbose=False):
        ...
        for file in files_list:
            input_image = os.path.join(folder, 'inputs2', file)
            target_image = os.path.join(folder, 'targets2' if self.include_hair else 'targets2', file)
